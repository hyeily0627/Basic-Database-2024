...
## 7일차 
- SQL 고급 
    - 트랜잭션 : All or Nothing🚨 로직 처리시 다른 트랜잭션의 간섭을 받지 않기 위한 것(LOCK), 중요한 데이터 수정시 잘못된 변경을 방지하기 위한 것
    ...
    - SQL 서버는 기본적으로 Auto COmmit(시스템이 자동으로 트랜잭션을 건다.)
    - <SETTING> SSMS > 도구 > 옵션 > 쿼리 실행 > SQL Server > ANSI 
      -> SET IMPLICIT_TRANSACTIONS 체크 


- 데이터 모델링 
    - <설계순서> 요구사항 수집 및 분석 - (개념설계 - 논리설계 - 물리설계) - 데이터베이스 구현 
        1) 개념 모델링 : 요구사항을 받으면서 정해지지 않은 여러 개체들을 정립화 하는 단계 
        2) 논리 모델링 : 기본키 지정, 외래키 지정, 관계 정립, 속성들 이름(한글) 개체를 정하는 단계 
        3) 물리 모델링 : DB에 맞춰서 컬럼 이름, 컬럼 데이터 타입 및 크기 지정, DB에 대한 검토로 테이블을 만들기 직전의 설계를 완성 

- ER 모델링(323p) : 세상의 사물을 개체(entity)와 개체 간의 관계로 표현 
...
- 인덱스 예제 
    ...
    <!-- ![설정](https://raw.githubusercontent.com/hyeily0627/Basic-Database-2024/main/images/db001.png) --> 
    <!-- CTRL + / ❗ 사용하여 주석처리 html태그 이미지 사이즈 조정 가능  -->
    <img src="https://raw.githubusercontent.com/hyeily0627/Basic-Database-2024/main/images/SSMS4.png)" width= "870">
...
- 정규화 : 이상현상(삽입, 삭제, 수정이상)이 발생하는 릴레이션을 분해하여 이상현상을 없애는 과정 
    - 기본키와 함수 종속성을 파악 
    - 정규형 
    ![정규화](https://raw.githubusercontent.com/hyeily0627/Basic-Database-2024/main/images/db002.png)

      1) 1정규형 : 속성 값이 원자값을 가지는 것 
      2) 2정규형 : 기본키가 아닌 속성이 기본키에 완전 함수 종속인 것(예 :  학생번호[PK],강좌이름 -> 성적을 결정 )
      3) 3정규형 : 기본키가 아닌 속성이 기본키에 비이행적으로 종속하는 것(직접종속)
         * 이행적 종속이란 A -> B, B -> C 성립시 A -> C가 성립되는 함수종속성 
      4) BCNF정규형 - 함수종속성 X -> Y가 성립할떄(3정규형 만족) 모든 결정자 X가 후보키(기본키가 될 수 있는 속성)인 것 
         -> 대부분의 릴레이션은 BCNF까지 정규화하면 실제적인 이상현상이 없어지기 떄문에 보통 여기까지 진행 
      5) 4정규형 : BCNF를 만족, 다치 종속(Multi-valued Dependency)이 없어야 함.
         * 다치 종속이란, 아래의 조건 만족할때 
         (1) A->B 일 때 하나의 A값에 여러 개의 B값이 존재하면 다치 종속성을 가진다고 하고 A↠B라고 표시한다
         (2) 최소 3개의 칼럼이 존재한다.
         (3) R(A, B, C)가 있을 때 A와 B 사이에 다치 종속성이 있을 때 B와 C가 독립적이다
      6) 5정규형 : 중복 제거를 위해 분해할 수 있을만큼 전부 분해하는 것 (조인 종속성, 무손실 분해)

- 실무실습(사용자,권한...)
    1. DB 관리자(SSMS에서 설정)
        - DB 관리자로 hr 데이터베이스 생성 및 관계를 설정함 
        - DB 관리자로 hrDB를 사용할 사용자 계정 생성 및 필요한 권한 설정
          
            - 추가❗sa의 비번을 잃어버리면, window인증으로 로그인 후 
            - SSMS > 보안 > 로그인 > sa 존재 > 속성에서 비밀번호 변경 이후 SQL Server인증으로 sa 비밀번호 입력 후 로그인 하면 됨 
            - SSMS > 보안 > 로그인 > 새 로그인 클릭 
                - ID: hr_user / pw: hr_p@ss!
                - 일반 : 기본 데이터베이스 hr 선택
                - 사용자 매핑 : hr 선택, 데이터베이스 역할 멤버 : db_owner추가 선택 
        - hrDB 계정 로그인
    ...
    ![HR_ERD](https://raw.githubusercontent.com/hyeily0627/Basic-Database-2024/main/images/db003.png)

## 9일차
- 실무실습
    - 쿼리실습하기 
        - 기본 SELECT, WHERE, ORDER BY 
        - 집계함수 : GROUP BY, ROLL UP 
        - JOIN, SUBQUERY, UNION...
        - CASE WHEN THEN END...
        - 내장함수... 
